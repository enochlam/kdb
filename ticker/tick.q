/ can't have -u because of the client end-of-day
...
/if[not"1"~first .z.l 4;'`license];"kdb+tick 2.4 2011.02.10"
...
\l tick/u.q
\d .u
ld:{if[not type key L::`$(-10_string L),string x;.[L;();:;()]];i::j::-11!(-2;L);hopen L};
...
if[system"t";
 ...
 upd:{[t;x]
 if[not -19=type first first x;if[d<"d"$a:.z.Z;.z.ts[]];a:"t"$a;x:$[0>type first x;a,x;(enlist(count first x)#a),x]];
 t insert x;if[l;l enlist (`upd;t;x);j+:1];}];

if[not system"t";system"t 1000";
 .z.ts:{ts .z.D};
 upd:{[t;x]ts"d"$a:.z.Z;
 if[not -19=type first first x;a:"t"$a;x:$[0>type first x;a,x;(enlist(count first x)#a),x]];
